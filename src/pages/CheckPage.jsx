...
const CheckPage = () => {
    ...
    useEffect(() => {
        ...
        depositor && recipient && recipient.recipient_district &&
            getOrderServices({
    ...
    }, [orderServices, recipientSelected])
                ...
                return (
                    <>
                        ...
                        <Flex gap={10}>
                            ...
                            <div>
                                {
                                    promotion ? <Flex gap={5}>
                                        <div>{FormatCurrency((record.original.price * (1 - (promotion.value / 100))) * record.original.quantity)}</div>
                                        <div className='old-price'>
                                            {FormatCurrency(record.original.price * record.original.quantity)}
                                        </div>
                                        <div><Badge count={`-${promotion.value}%`} /></div>
                                    </Flex>
                                        : FormatCurrency(record.original.price)
                                }
                            </div>
                        </Flex>
                    </>
    ...
    const onFinish = () => {
        const order_details = cartItemSelected.map((item) => {
            ...
            return {
                ...
                unit_price: promotion ? item.price * (1 - (promotion.value / 100)) : item.price,
        ...
        const order = {
            ...
            recipient_district: recipient.recipient_district,
            recipient_ward: recipient.recipient_ward,
            recipient_detail: recipient.recipient_detail,
